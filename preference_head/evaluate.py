...
def main(A: str, B: str) -> None:
    with open("chatbot_arena_llama_sample1.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    with open("chatbot_arena_chatgpt.jsonl", "r", encoding="utf-8") as f:
        data_b = [json.loads(line) for line in f if line.strip()]
    OUT_PATH = Path(f"Asample1_vs_Bgpt"+"_pairwise.jsonl")
